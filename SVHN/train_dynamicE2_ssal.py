...
from dataset import SVHNDataset,SVHNDatasetAL
from dataloader import ComposeLoaderLU

parser = argparse.ArgumentParser(description="SVHN SSAL Running")
...
parser.add_argument('--E1', default=11, type=int)
...
n_classes = 10 # total class number

DEVICE = torch.device('cuda' if torch.cuda.is_available() else 'cpu')

train_loss_file = "/home/wyx/vscode_projects/SSAL/models/SVHN/DynamicE2/{}-{}/train_loss.csv".format(args.info, args.balance)
val_loss_file = "/home/wyx/vscode_projects/SSAL/models/SVHN/DynamicE2/{}-{}/val_loss.csv".format(args.info, args.balance)
test_acc_file = "/home/wyx/vscode_projects/SSAL/models/SVHN/DynamicE2/{}-{}/test_acc.csv".format(args.info, args.balance)
states_file_path = "/home/wyx/vscode_projects/SSAL/models/SVHN/DynamicE2/{}-{}/states.json".format(args.info, args.balance)
best_model_path = "/home/wyx/vscode_projects/SSAL/models/SVHN/DynamicE2/{}-{}/best_model.pth".format(args.info, args.balance)
...
# 读取数据
dataset_L, dataset_U = SVHNDatasetAL.createLURandomly(nL=500, image_size=32)
train_loader_LU = ComposeLoaderLU(dataset_L, dataset_U, BATCH_SIZE)
dataset_test = SVHNDataset(train=False, image_size=32)
...
#rho_delta_list = []
best_acc = 0
# initial labeled dataset, only one training epoch
# Record the number of training epochs after each AL round.
E2s = [1] 
for i in range(1, E1): # [1, E1-1]
    E2s.append(int(np.ceil(i * E2 / (E1-1))))
print("E2: ", E2s)
epoch = 1
for i, E2i in enumerate(E2s):
    print("Current AL round: ", i)
    print("Current number of training epochs: ", E2i)
    # training 
    for _ in range(E2i):
        print("Training....")
        train(epoch)
